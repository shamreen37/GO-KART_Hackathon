<!doctype html>
<html lang="en">

<head>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">


    <link rel="shortcut icon" href="assets/favicon.png" type="assets/x-icon" />
    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="Css/bootstrap.css">
    <link rel="stylesheet" href="Css/style2.css">
    <link rel="stylesheet" href="Css/style.css">
    <link rel="stylesheet" href="Css/media.css">
    <link rel="stylesheet" href="Css/data-aos.css">
    <link rel="stylesheet" href="Css/style3.css">
    <title>GoKart-Gallery</title>
</head>

<body>
    <!--NAVBAR -->
    <section class="navbar-section">
        <div class="navbar-container nav-flex container-fluid">
            <div class="logo">
                <img src="assets/logo-img.png" alt="" class="logo">

            </div>
            <div class="navbar">

                <div class="icon-bar" onclick="Show()">
                    <i></i>
                    <i></i>
                    <i></i>
                </div>

                <ul id="nav-lists">
                    <li class="close"><span onclick="Hide()">×</span></li>
                    <li><a class="underline-hover-1" href="index.html">Home</a></li>
                    <li><a class="underline-hover-1" href="about.html">About</a></li>
                    <li><a class="underline-hover-1" href="events.html">Events <span class="new">New</span></a></li>
                    <li><a class="underline-hover-1" href="news.html">News</a></li>
                    <li><a class="underline-hover-1" href="gallery.html">Gallery</a></li>
                    <li><a class="underline-hover-1" href="campus.html">Campus and Facilities</a></li>
                    <li><a class="underline-hover-1" href="contact.html">Contact</a></li>
                </ul>

            </div>
        </div>
    </section>


    <!--================================================================-->
    <!--- GALLERY -->
    <section class="image-set">
        <div class="container mx-lg-auto">

            <h4 class="fw-bold fs-3 py-3 mb-3 text-dark gradient-underline font1">GALLERY</h4>

            <div class="row gx-lg-5 gx-md-5 gx-sm-1">
                <div class="col-12 col-lg-4 col-md-4">
                    <div class="row my-4">

                        <div class="card shadow">
                            <div class="gallery-image" data-aos="fade-up">
                                <img src="https://source.unsplash.com/srpE-a5W418/400x300" class="img img-fluid rounded">
                            </div>
                            <div class="gallery-content">
                                <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Officia, unde?</p>
                            </div>
                        </div>
                    </div>
                    <div class="row my-4">

                        <div class="card shadow" data-aos="fade-up">
                            <div class="gallery-image">
                                <img src="https://source.unsplash.com/moX0iBfnCs0/400x400" class="img img-fluid rounded">
                            </div>
                            <div class="gallery-content">
                                <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Officia, unde?</p>
                            </div>
                        </div>
                    </div>
                    <div class="row my-4">

                        <div class="card rounded shadow" data-aos="fade-up">
                            <div class="gallery-image">
                                <img src="https://source.unsplash.com/FBTcERIsc5E/400x500" class="img-img-fluid rounded">
                            </div>
                            <div class="gallery-content">
                                <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Officia, unde?</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-lg-4 col-md-4">
                    <div class="row my-4">

                        <div class="card rounded shadow" data-aos="fade-up">
                            <div class="gallery-image">
                                <img src="https://source.unsplash.com/itC4cdcVC6I/400x500" class="img img-fluid rounded">
                            </div>
                            <div class="gallery-content">
                                <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Officia, unde?</p>
                            </div>
                        </div>
                    </div>
                    <div class="row my-4">

                        <div class="card rounded shadow" data-aos="fade-up">
                            <div class="gallery-image">
                                <img src="https://source.unsplash.com/2icCWJXwOis/400x400" class="img img-fluid rounded">
                            </div>
                            <div class="gallery-content">
                                <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Officia, unde?</p>
                            </div>
                        </div>
                    </div>
                    <div class="row my-4">

                        <div class="card rounded shadow" data-aos="fade-up">
                            <div class="gallery-image">
                                <img src="https://source.unsplash.com/eP9pqF1WTlo/400x300" class="img-img-fluid rounded">
                            </div>
                            <div class="gallery-content">
                                <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Officia, unde?</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-lg-4 col-md-4">
                    <div class="row my-4">

                        <div class="card shadow rounded" data-aos="fade-up">
                            <div class="gallery-image">
                                <img src="https://source.unsplash.com/SdNimAHY1Gw/400x300" class="img img-fluid rounded">
                            </div>
                            <div class="gallery-content">
                                <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Officia, unde?</p>
                            </div>
                        </div>
                    </div>
                    <div class="row my-4">

                        <div class="card shadow rounded" data-aos="fade-up">
                            <div class="gallery-image">
                                <img src="https://source.unsplash.com/nvzXUaV8uX4/400x400" class="img img-fluid rounded">
                            </div>
                            <div class="gallery-content">
                                <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Officia, unde?</p>
                            </div>
                        </div>
                    </div>
                    <div class="row my-4">

                        <div class="card shadow rounded" data-aos="fade-up">
                            <div class="gallery-image">
                                <img src="https://source.unsplash.com/UuCi7XKh0jU/400x500" class="img-img-fluid rounded">
                            </div>
                            <div class="gallery-content">
                                <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Officia, unde?</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!--FOOTER-->

    <footer class="footer">

        <div class="container footer-container pt-5">
            <div class="row ">
                <div class="col-md-4 col-sm-6 col-xs-12 text-light ">
                    <div class="row-col">
                        <div class="footer-logo-img">
                            <img src="assets/logo-img.png" alt="" class="logo">
                        </div>
                        <p class="pt-3 footer-logo-txt">
                            Lorem, ipsum dolor sit amet consectetur adipisicing elit. Exercitationem beatae quos praesentium, possimus iusto sequi accusamus error eius voluptas ab perspiciatis sed omnis quam dolore.
                        </p>

                    </div>
                </div>

                <div class="col-md-2 col-sm-6 col-xs-12 px-2 text-light">
                    <div class="row-col">
                        <p class="topic text-light pb-3">Quick Links</p>
                        <p><a href="#">Home</a></p>
                        <p><a href="#about">About Us</a></p>
                        <p><a href="#gallery">Gallery</a></p>
                        <p><a href="#news">Latest News</a></p>
                        <p><a href="contact.html">Contact Us</a></p>
                    </div>
                </div>

                <div class="col-md-3 col-sm-6 col-xs-12 px-2 text-light">
                    <div class="row-col">
                        <p class="topic text-light pb-3">Contact us</p>
                        <p><i class="fas fa-phone-alt"></i> Phone:</p>
                        <p>+91 4684684111</p>
                        <p>+91 7866000000</p>
                        <p><i class="fas fa-envelope"></i> Email:</p>
                        <p><a>example@gmail.com</a></p>
                    </div>
                </div>

                <div class="col-md-3 col-sm-6 col-xs-12 px-2 text-light">
                    <div class="row-last-col">
                        <p class="topic pb-3">Newsletter</p>

                        <form class="d-flex" onsubmit="subscribeNewsletter(event,this)">
                            <input class="form-control" name="subsForNewsletter" type="email" placeholder="Email" required>
                            <button class="btn btn-light" type="submit"><i class="fab fa-telegram-plane"></i></button>
                        </form>
                        <div class="social-div">
                            <p class="pt-4 topic">Follow Us</p>

                            <a href="" class="p-1"><i
                                class="fab fa-instagram"></i> </a>
                            <a href="" class="p-1"><i
                                class="fab fa-facebook-f"></i> </a>
                            <a href="" class="p-1"><i
                                class="fab fa-linkedin-in"></i> </a>
                        </div>
                    </div>
                </div>


            </div>

        </div>


        <div class="copyright text-center p-3">
            <p> Copyright © 2021 All Rights Reserved by Go-Kart.</p>
        </div>
    </footer>



    <!-- JS-->


    <script src="js/bootstrap.js"></script>
    <script src="js/jquery.js"></script>
    <script src="js/owl-carousel.js"></script>
    <script src="js/slider.js"></script>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.3/css/all.min.css" integrity="sha512-iBBXm8fW90+nuLcSKlbmrPcLa0OT92xO1BIsZ+ywDWZCvqsWgccV3gFoRBv0z+8dLJgyAHIhR35VZc2oM/gI1w==" crossorigin="anonymous" referrerpolicy="no-referrer"
    />
    <script>
        var navList = document.getElementById("nav-lists");

        function Show() {
            navList.classList.add("_Menus-show");
        }

        function Hide() {
            navList.classList.remove("_Menus-show");
        }
    </script>
    <script src="https://unpkg.com/aos@next/dist/aos.js"></script>
    <script>
        AOS.init({
            offset: 250,
            duration: 1000,
        });
    </script>

</body>

</html>